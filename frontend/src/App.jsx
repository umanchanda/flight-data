...
function FlightTable({ flights }) { return <div className="table-wrap"><table><thead><tr><th>Date</th><th>Flight</th><th>Route</th><th>Airline</th><th>Aircraft</th><th>Duration</th><th>Class</th></tr></thead><tbody>{flights.map((flight) => <tr key={flight.id}><td>{formatDate(flight.date)}</td><td className="mono">{flight.flight_number || "—"}</td><td><strong>{shortAirport(flight.from_airport)}</strong> <span className="route-arrow">→</span> <strong>{shortAirport(flight.to_airport)}</strong></td><td>{flight.airline}</td><td>{flight.aircraft || "—"}</td><td>{toHours(flight).toFixed(1)} h</td><td><span className="tag">{flight.flight_class || "—"}</span></td></tr>)}</tbody></table>{!flights.length && <div className="empty">No flights match these filters.</div>}</div>; }
